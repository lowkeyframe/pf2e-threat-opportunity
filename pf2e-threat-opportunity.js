 ...
 */
Hooks.on('renderChatMessage', async (message, html, data) => { // Made async for ChatMessage.create
    // Prevent processing messages generated by this module itself to avoid infinite loops.
    if (message.getFlag("pf2e-threat-opportunity", "generated")) {
        return;
    }
    ...
    const hasAlreadyProcessed = message.getFlag("pf2e-threat-opportunity", "processed"); // This flag is for the original message to prevent multiple new messages
    ...
    // If a detail was determined, create a new chat message.
    if (detailText) {
        // Determine the roll type for the new message content
        let rollType = "Roll"; // Default
        if (context?.type) {
            // Capitalize and space out types like "skill-check" -> "Skill Check"
            rollType = context.type.split('-').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join(' ');
        } else if (message.flavor) {
            rollType = message.flavor; // Use the flavor text if available
        } else if (message.content.includes("PF2E.Roll.")) {
            // A more generic fallback if context.type and flavor aren't specific
            rollType = "PF2e Roll";
        }

        const newMessageContent = `
            <div class="pf2e-threat-opportunity-new-message">
                <p>For your <strong>${rollType}</strong> (Result: ${rollTotal}, DC: ${dc}):</p>
        ...
        await ChatMessage.create({
            ...
            speaker: ChatMessage.getSpeaker({ actor: message.actor }), // Associate with the actor who made the roll
            type: CONST.CHAT_MESSAGE_TYPES.OTHER, // Use 'OTHER' for custom messages
            flags: {
                "pf2e-threat-opportunity": {
                    generated: true // Flag to prevent this new message from triggering the hook again
                },
                // Optionally link to the original message for easier context/linking
                "core.origin": message.id
            }
        });

        // Set a flag on the original message to indicate it has been processed by this module.
        // This prevents multiple new messages from being created for the same original roll if renderChatMessage is called multiple times.
        message.setFlag("pf2e-threat-opportunity", "processed", true);
        console.log(`PF2e Threat & Opportunity | Created new message with '${detailText}' for original message ${message.id}.`);
    }
});
